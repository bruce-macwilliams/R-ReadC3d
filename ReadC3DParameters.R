# ..............................................................................
# ReadC3DParameters.R for public release. 
# For readme and licensing information see the main file ReadC3D.R
# This file contains three functions called by the main ReadC3D() function in ReadC3D.R
# 1. ReadC3DParameters(con)
#    This is the top level subfunction that is used to read the entire C3D parameter section
#    The single passed argument 'con' is the connection to the C3D file (binary file pointer location).
#    Note the ReadC3D() only calls this once so there is not need for it to be a separate function. 
#    I did it this way to make debugging and editing easier than it would be if these line were buried 
#    in the main code. This is the most complex part of the entire code, so it seemed like a good idea to keep it separate.
#    
#    Function returns a data frame with columns
#       Group: group number (numerical) 
#       Parameter: paramter number (numerical)
#       Group Name: (character)
#       Parameter Name: (character)
#       NumDimensions: Number of dimensions (nummeric 0-3) where 
#          0 is a single value
#          1 is a one dimensional array
#          2 is a 2D array
#          3 is a 3D array
#       Dimensions: Lengths of dimensions (list: numeric for NumDimensions = c(0,1), character array for NumDimensions = c(2,3))
#       as an example a 2D array from the sample file included for the parameter POINT:LABELS has a value c(" 30", "171") 
#       DataType (numeric)
#          -1 = character
#           1 = byte
#           2 = integer
#           4 = float/real
#       Data: The values for the parameter passed in a list with each observation the DataType type with dimensions from Dimensions
# 
# 2. GetParameterData(P, GName, PName, Warn = TRUE)
#    This function allows for the retrieval of a single parameter by name where
#       P is the parameter data frame that has been returned from P <- ReadC3DParameters(con)
#       GName (string) is valid parameter group name, e.g. 'POINT'
#       PName (string) is a valid parameter name within the specified group, e.g. 'FRAMES'
#       warn (logical) is a switch to control warning messages generated within the function
#
#   Function returns the Data of the Parameter data frame values of the specified type and dimensions
#
# 3. DECBytes2Single(dec_bytes) 
#    This is s conversion function called by ReadC3d when DEC storage is identified.
#    Swaps the byte order and creates a valid numeric value
# ..............................................................................
...
GetParameterData <- function(P, GName, PName, Warn = TRUE) {
  ...
  if (is_empty(PR)) {
    ...
    # Otherwise get parameter dimensions and values
    dims <- unlist(P$Dimensions[PR])
    pdata <- unlist(P$Data[PR])
    
    # If a matrix or 3D array of numbers reformat
    # if string
    if (P$DataType[PR] == -1) {
...
# Probably SGI formats could be read with a similar function
...
ReadC3DParameters <- function(con) {
  ...
  while (GroupNumber != 0) {
    ...
    if (GN < 0) {
      ...
      PR <- PR + 1
      # Note it would be faster to do this in a list, but it's already done ...
