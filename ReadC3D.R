...
################################################################################
# Note the main function requires:
#    1. dplyr, you can use library(dplyr) or others but I usually go with library(tidyverse)
# In addition this function and dependency must be sourced prior to function call
#    2. source("ReadC3D.R")
#    3. source("ReadC3DParameters.R")
################################################################################
...
ReadC3D <- function(
  ...
  # I believe that SGI simply requires reversal of bytes, high byte first, low byte second
  # so this certainly could be implemented but this is ancient technology,
  ...
  # Word 10: Number of analog frames per video frame, unsigned 2 byte integer
  ...
  if (EventFlag == 12345) {
    ...
    # Split string into vector of 4 character substrings
  ...
  # Handle files where number of frames exceed integer storage as read in the header.
  ...
  if (VideoData) {
    ...
    # whitespace to break up strings, need to use dimensions and these can 
  ...
  if (AnalogData) {
    ...
    # MAC Systems apparently label this as "OFFSETS" (way to go off label MAC!)
    ...
    # Offsets should always be positive values. This is quite a cluster (never
    # should have been allowed) and only applies to one(!) file of >100 tested. 
  ...
  #   All Analog data for that frame, depends on Analog:Video ratio
  ...
  # Note these warnings may be redundant or unnecessary at this point, but I'm no programmer
  if (VideoData) {
  ...
  # 1) VideoData: All marker and calculated 3D variables
  # 2) ADCData: All analog channels
  # Start by making a matrix with rows (observations) of frame numbers and columns will be 3D data 
  # and every analog channel having NumberOfAnalogChannelsPerFrame columns
  VideoMatrix <- matrix(VideoBytes, nrow = NumberOfC3DFrames, ncol = VidCols, byrow = TRUE)
  # VideoBytes no longer needed, clear space
  rm(VideoBytes)
  # Now split this matrix of everything into Marker and ADC matrices
  
  #### ---- Create Video Data Frame ----
  # Video data in columns of NumberOfMarkers by rows (observations) of NumberOfC3DFrames
  ...
  if (AnalogData) {
    ...
    # Videomatrix no longer needed, remove for space
    rm(VideoMatrix)
    ...
    # Permutate so that sub-frames are first dimension (aperm() rocks!)
    ...
    # Make ADCData into a data frame with 3 ID columns: 
    #   ADC frames (each observation), 
    #   Video frames, 
    #   Sub-frames 
    ...
    # Fix any issue with non-unique names
    AnalogNames <- make.names(AnalogNames, unique = TRUE)
    # SubFrames = sub samples of analog data for each video frame
    # e.g. if the video is at 10 Hz and analog at 1000, then there is a 10:1
    # ration of analog:video so analog will have 10 subframes for each video frame 
  ...
  # Only run this if 1) asked for and 2) there is analog data ...
  if (GetForcePlateData & AnalogData) {
    ...
    if (FP_Used == 0) { 
      ...
      for (FP in 1:FP_Used) {
        ...
        if (Offset) {
          # If frame range is valid, remove "DC" noise. e.g. Valid: FP_Zero = (1, 10)
          # If frame range is invalid, leave the signal alone. e.g. invalid FP_Zero = (10, 1)
        ...
        # Store standardized force plate data in data frame FPDataT
        # Tall data frame of 6 outputs Fx, Fy, Fz, COP_x, COP_y, Tz
        ...
        # Type 4 Force Plates
          ...
          # Load current forces, center of pressure, and free torque into FPDataT data frame
          # Since these are individual arrays here, create data frame
        ...
        # set an artificial threshold of 1N, so zero to at least below 1N
  ...
  if (GaitCycleEventsUsed == 0) {
    ...
    # Each time is stored as minutes in one field, seconds in next field
  ...
  # All C3D files should have these elements
  C3D <- list(
    'Header' = Header,
    'Events' = Events,
